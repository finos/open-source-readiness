...
In general, the OSR community seem agreed on method 3 for tracking firm contribitutions.

:::

## Enterprise Managed User (EMU) Accounts

- One approach is that you have an EMU account (an account for the enterprise github) and Public account. A public contribution needs to acknowledge your role. A tool maps internal users against public ones. You make changes to the internal fork, but then an internal process shuttles the commits to the public fork. The email from the internal system must also exist on the public github account.
...
Organizations must establish clear policies for reviewing and managing personal open source contributions. Key considerations include:

- All company-related contributions must undergo formal review processes
- Contributions made during work hours or using company resources are typically considered company-scoped
- Clear guidelines should define what constitutes company-scoped work
- Prohibiting personal open source work on company hardware
- Restricting use of company email addresses for personal projects

### Personal Project Policies

Organizations typically implement one of two approaches:

1. **Per-Project Approval**

   - Maintaining a list of approved/denied projects -OR-
   - Requiring declaration as an outside business interest
   - Requiring formal approval for new project contributions
     Example: An employee contributing to an open source beer-making project would need approval if not on the deny list

2. **Restricted Access For Classes Of Projects**

   - If a project cannot be described as "firm business" personal contribution is allowed automatically.
   - This would rule out contributing to any other finance-related OSS.
